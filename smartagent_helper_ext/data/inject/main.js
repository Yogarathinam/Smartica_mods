...
// --- SMARTAGENT JQUERY KILL SWITCH ---
// This runs natively inside all iframes, completely bypassing CSP and destroying trackers.
const killTrackers = setInterval(() => {
    if (typeof window.$ !== 'undefined') {
        try {
            window.$(window).trigger('focus'); // Force the site to think you're looking at it
            window.$(window).off('blur focus'); // Delete the away tracker
            console.log("✅ jQuery trackers successfully destroyed in frame:", window.location.hostname);
            clearInterval(killTrackers);
        } catch(e) {}
    }
}, 500);

// Stop checking after 10 seconds to save memory
setTimeout(() => clearInterval(killTrackers), 10000);